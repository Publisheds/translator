...
const jsxbin_path = path.join(__dirname, 'jsxbin.js')
...
function build() {
    try {
        ...
        utils.log_progress('convert to jsxbin==>END')
        execSync(`webpack --config ${webpack_client_config_path} --env target=web --mode ${env}`, { stdio: [0, 1, 2] })
        // bundle the session
        ...
        fs.mkdirSync(xml_out_dir, { recursive: true });
